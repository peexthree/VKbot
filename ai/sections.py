...
async def generate_section(section: str, date: str, time: str, city: str, core_profile: str = "", first_name: str = "", sex: int = 0, partner_name: str = "", partner_date: str = "", skin: str = "olesya", card_id: str = None, card_data: dict = None, tags: list = None, return_json: bool = False) -> str | dict | None:
    ...
    base_info += " ОБЯЗАТЕЛЬНО используй слово ВСТУПЛЕНИЕ на отдельной строке перед вступительной частью."
    ...
    style_instruction = (
        ...
        "Текст должен разбиваться на мягкие абзацы по 2-3 предложения."
    )

    if section == "base":
        prompt = f"{base_info} Составь ВСТУПЛЕНИЕ (2-3 предложения, максимально лаконично) и ИСТОКИ (разбор Солнца, Луны и Асцендента, глубокий и подробный анализ). ОБЯЗАТЕЛЬНО используй слово ИСТОКИ на отдельной строке перед основным разбором. Выдели заголовки ВСТУПЛЕНИЕ и ИСТОКИ КАПСОМ.{style_instruction}"
    elif section == "sex":
        cid = card_id if card_id else random.choice(list(range(22, 50)))
        prompt = f"{base_info} Сделай ВСТУПЛЕНИЕ (2-3 предложения, максимально лаконично) и разбор СТРАСТЬ (анализ Венеры и Марса, отношение к любви и близости, глубокий и подробный анализ). ОБЯЗАТЕЛЬНО используй слово СТРАСТЬ на отдельной строке перед основным разбором. Выдели заголовки ВСТУПЛЕНИЕ и СТРАСТЬ КАПСОМ. В самом конце текста ОБЯЗАТЕЛЬНО добавь строку с ID карты Таро в формате: ID_ТАРО: {cid}. Вплети этот ID прямо в свой прогноз.{style_instruction}"
    elif section == "money":
        cid = card_id if card_id else random.randint(64, 77)
        prompt = f"{base_info} Сделай ВСТУПЛЕНИЕ (2-3 предложения, максимально лаконично) и разбор ИЗОБИЛИЕ (анализ 2-го и 10-го домов, самореализация и процветание, глубокий и подробный анализ). ОБЯЗАТЕЛЬНО используй слово ИЗОБИЛИЕ на отдельной строке перед основным разбором. Выдели заголовки ВСТУПЛЕНИЕ и ИЗОБИЛИЕ КАПСОМ. В самом конце текста ОБЯЗАТЕЛЬНО добавь строку с ID карты Таро в формате: ID_ТАРО: {cid}. Вплети этот ID прямо в свой прогноз.{style_instruction}"
    elif section == "shadow":
        cid = card_id if card_id else random.randint(50, 63)
        prompt = f"{base_info} Сделай ВСТУПЛЕНИЕ (2-3 предложения, максимально лаконично) и разбор ТЕНЬ (анализ Лилит и Селены, скрытые уголки души, глубокий и подробный анализ). ОБЯЗАТЕЛЬНО используй слово ТЕНЬ на отдельной строке перед основным разбором. Выдели заголовки ВСТУПЛЕНИЕ и ТЕНЬ КАПСОМ. В самом конце текста ОБЯЗАТЕЛЬНО добавь строку с ID карты Таро в формате: ID_ТАРО: {cid}. Вплети этот ID прямо в свой прогноз.{style_instruction}"
    elif section == "final":
        cid = card_id if card_id else random.randint(0, 21)
        prompt = f"{base_info} Сделай ВСТУПЛЕНИЕ (2-3 предложения, максимально лаконично) и ПУТЬ (Итоговое напутствие в жизни и светлый совет для души, глубокий и подробный анализ). ОБЯЗАТЕЛЬНО используй слово ПУТЬ на отдельной строке перед основным разбором. Выдели заголовки ВСТУПЛЕНИЕ и ПУТЬ КАПСОМ. В самом конце текста ОБЯЗАТЕЛЬНО добавь строку с ID карты Таро в формате: ID_ТАРО: {cid}. Вплети этот ID прямо в свой прогноз.{style_instruction}"
    elif section == "synastry":
        cid = card_id if card_id else random.randint(0, 21)
        prompt = f"{base_info} Сделай профессиональный разбор вашей связи (СОЮЗ). Имя партнера: {partner_name}, полные данные рождения партнера (дата, время, город): {partner_date}. ВСТУПЛЕНИЕ сделай максимально коротким (2-3 предложения, без лишней воды). В основном блоке СОЮЗ проведи глубокий синастрический анализ вашей совместимости, используя предоставленные данные обоих партнеров. Опиши магию вашего мэтча, кармические точки соприкосновения и уроки, которые вы несете друг другу. Обязательно удели внимание сексуальной совместимости и потенциалу развития отношений. Твой стиль должен быть точным, как в SaaS-продукте, но сохранять эзотерическую глубину. ОБЯЗАТЕЛЬНО используй слово СОЮЗ на отдельной строке перед основным разбором. Выдели заголовок СОЮЗ КАПСОМ. В самом конце текста ОБЯЗАТЕЛЬНО добавь строку с ID карты Таро в формате: ID_ТАРО: {cid}. Вплети этот ID прямо в свой прогноз.{style_instruction}"
