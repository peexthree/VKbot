...
async def process_synastry_date(message: Message):
    ...
    try:
        ...
        await bot.api.messages.send(peer_id=vk_id, message="Данные приняты.", random_id=0)

        kb = Keyboard(inline=True)
        kb.add(Callback("✦ ОПЛАТИТЬ И РАСКРЫТЬ", payload={"cmd": "pay_and_reveal", "key": "synastry", "partner_name": partner_name, "partner_date": partner_date}), color=KeyboardButtonColor.POSITIVE)
        await bot.api.messages.send(peer_id=vk_id, message=f"Ритуал Синастрии готов. Стоимость: 1500 Энергии звезд.\n\nЖми кнопку ниже, чтобы начать таинство.", keyboard=kb.get_json(), random_id=0)
    except Exception as e:
        logger.error(f"Ошибка в process_synastry_date: {str(e)}")
    finally:
        await release_lock(vk_id)
