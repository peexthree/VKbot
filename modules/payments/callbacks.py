...
from vkbottle import Callback, GroupEventType, Keyboard, KeyboardButtonColor, OpenLink
...
from database import get_user, set_user_state, update_user, save_feedback
from modules.bot_init import bot
from modules.keyboards import rating_keyboard, feedback_skip_keyboard
...
async def _message_event_handler_wrapped(event: dict, skip_lock: bool = False):
    ...
    if event_id and vk_id and not skip_lock:
        ...
        if cmd == "skin_quest":
            ...
            clean_quest = await get_short_quest_text(vk_id, payload.get("skin"))
            ...
            if user:
                ...
                if latest_text:
                    ...
                    clean_text = re.sub(
                        r"^(ХИРОМАНТИЯ|СОННИК|КАРТА ДНЯ|СЕКСУАЛЬНОСТЬ|БОГАТСТВО|ТЕНЬ|ПУТЬ|СИНАСТРИЯ|ОРАКУЛ|АНТИТАРО|РАЗБОР)\s+",
                        "",
                        latest_text,
                        flags=re.IGNORECASE,
                    ).strip()
            ...
            await bot.api.messages.send_message_event_answer(
                ...
                event_data=json.dumps(
                    {"type": "show_snackbar", "text": "Карточка для пересылки готова!"}
                ),
            )

            # 2. Формируем виральное сообщение
            ref_link = (
                f"https://vk.me/club219181948?ref=result_{card_id}"
                if card_id
                else "https://vk.me/club219181948?ref=result"
            )
            ...
            photo_file = (
                f"{card_id}.jpeg"
                if card_id and os.path.exists(os.path.join("cards", f"{card_id}.jpeg"))
                else "uslugi/main_menu.jpeg"
            )
            ...
            elif action == "grimoire" or cmd in [
                "grimoire_page",
                "grimoire_cards_list",
            ]:
            ...
            elif action == "change_skin" or cmd in [
                "hall_of_prophets",
                "skin_page",
                "skins_page",
            ]:
            ...
            try:
                if nav_msg:
                    await bot.api.messages.send_message_event_answer(
                        ...
                        peer_id=peer_id,
                        event_data=json.dumps(
                            {"type": "show_snackbar", "text": nav_msg},
                            ensure_ascii=False,
                        ),
                    )
                else:
        ...
        logger.warning(
            f"Event {event_id} received without vk_id, cannot remove spinner"
        )
    ...
    if not skip_lock:
        ...
        if not await acquire_lock(vk_id, ttl=5):
            logger.warning(
                f"Could not acquire lock for vk_id={vk_id} in message_event_handler"
            )
            return
    try:
        ...
        if user_for_stats:
            ...
            if last_stats_at_str:
                last_stats_at = datetime.datetime.fromisoformat(
                    last_stats_at_str.replace("Z", "+00:00")
                )
                diff_stats = (now_stats - last_stats_at).total_seconds()
                # Если прошло меньше 10 минут, считаем это одной сессией
                if diff_stats < 600:
                    p_stats["stats_total_seconds"] = (
                        p_stats.get("stats_total_seconds", 0) + diff_stats
                    )
        ...
        if cmd == "whisper_fomo_reverify":
            await safe_edit(
                peer_id=peer_id,
                message="✦ ВОССТАНОВЛЕНИЕ КАНАЛА СВЯЗИ...",
                conversation_message_id=obj.get("conversation_message_id"),
            )
            try:
            ...
            if original_intent:
                new_state["original_intent"] = original_intent
            await set_user_state(vk_id, json.dumps(new_state))
            await safe_edit(
                peer_id=peer_id,
                message="🔮 ДАННЫЕ СТЕРТЫ В ЦЕЛЯХ БЕЗОПАСНОСТИ\n\nДавай попробуем еще раз. Напиши свою ДАТУ рождения (например, 15.04.1990):",
                conversation_message_id=obj.get("conversation_message_id"),
            )
            ...
            await safe_edit(
                peer_id=peer_id,
                message="Хорошо, давай начнем сначала. Напиши свою ДАТУ рождения (например, 15.04.1990):",
                conversation_message_id=obj.get("conversation_message_id"),
            )
            ...
            await safe_edit(
                peer_id=peer_id,
                message="✦ СИНХРОНИЗАЦИЯ С МАТРИЦЕЙ...",
                conversation_message_id=obj.get("conversation_message_id"),
            )
            ...
            await set_temp_birth_data(vk_id, {"date": date, "time": time, "city": city})

            core_profile = f"{date} {time} {city}"
            await redis_client.set(f"user:core_profile:{vk_id}", core_profile, ex=86400)

            user = await get_user(vk_id)
            updates = {"is_registered": True, "core_profile": core_profile}
            ...
            if original_intent:
                ...
                if oi_cmd in [
                    ...
                    return await _message_event_handler_wrapped(
                        event_for_oi, skip_lock=True
                    )
                elif oi_cmd == "process_payment_and_generate":
                    return await process_payment_and_generate(
                        vk_id, original_intent.get("section")
                    )
            ...
            if target_section := state_dict.get("target_section"):
                ...
                await safe_edit(
                    peer_id=peer_id,
                    conversation_message_id=obj.get("conversation_message_id"),
                    message="✨ ДАННЫЕ ПРИНЯТЫ. ТЕПЕРЬ ШАГ 2 ИЗ 3: СИНХРОНИЗАЦИЯ.\n\nЖми кнопку ниже.",
                    keyboard=kb.get_json(),
                )
                return
            ...
            if user and target_section:
                ...
                if has_access:
                    if target_section == "synastry":
                        await set_user_state(
                            vk_id, json.dumps({"step": "waiting_synastry_name"})
                        )
                        await safe_edit(
                            peer_id=peer_id,
                            conversation_message_id=obj.get("conversation_message_id"),
                            message="ДЛЯ АНАЛИЗА СОЮЗА НАПИШИ ИМЯ ПАРТНЕРА:",
                        )
                        return
                    if target_section == "dream":
                        await set_user_state(
                            vk_id, json.dumps({"step": "waiting_dream_text"})
                        )
                        msg = (
                            "✅ Оплата прошла.\n\n"
                    ...
                    if target_section == "palmistry":
                        await set_user_state(
                            vk_id, json.dumps({"step": "waiting_palmistry_photos"})
                        )
                        msg = (
                    ...
                    if target_section == "oracle":
                        ...
                        await set_user_state(
                            vk_id, json.dumps({"step": "waiting_oracle_question"})
                        )
                        await safe_edit(
                            peer_id=peer_id,
                            conversation_message_id=obj.get("conversation_message_id"),
                            message="НАПИШИ СВОЙ ВОПРОС СУДЬБЕ:",
                        )
                        return
                    ...
                    await show_services(
                        ...
                        peer_id,
                        0,
                        edit_msg_id=obj.get("conversation_message_id"),
                        is_catalog=True,
                        target_key=target_section,
                    )
        elif cmd == "main_menu":
            ...
            moons = ["🌑", "🌘", "🌗", "🌖", "🌕", "✨", "🔥"]
            streak_visual = "".join(
                moons[i % len(moons)] if i < visit_streak else "○" for i in range(7)
            )
            ...
            att = await upload_local_photo(
                bot.api, "uslugi/main_menu.jpeg", peer_id=vk_id
            )
        ...
        elif cmd in ["services_menu", "secret_arts_menu"]:
            ...
            # Проверяем, есть ли уже купленный доступ или VIP-безлимит для услуг
            user = await get_user(vk_id)
            if user:
                purchased = user.get("purchased_sections", {})
                has_access = purchased.get(key, False)
                if key in ["sex", "money", "shadow", "final"]:
                    if purchased.get("all") or user.get("has_full_chart"):
                        has_access = True
                elif key == "oracle":
                    has_access = purchased.get("oracle_access", False)

                from modules.utils.logic import is_vip_unlimited

                # Сонник и Хиромантия бесплатны для VIP-безлимита
                if key in ["palmistry", "dream"] and is_vip_unlimited(user):
                    has_access = True

                if has_access:
                    # Сразу запускаем услугу!
                    # Для этого мы просто имитируем вызов use_section
                    payload_use = {"cmd": "use_section", "key": key}
                    event_copy = event.copy()
                    event_copy["object"]["payload"] = payload_use
                    return await _message_event_handler_wrapped(
                        event_copy, skip_lock=True
                    )

            item_data = {
                "oculomancy": (
                    "👁 ОКУЛОМАНТИЯ: ЗЕРКАЛО ДУШИ",
                    "Пользователь загружает крупное фото своего глаза. ИИ считывает оттенок, узоры радужки и форму разреза, выдавая глубокий психологический портрет, скрытые таланты и «уязвимости» души.\n\nСтоимость: 1200 ✨",
                    "biometrics",
                    1200,
                ),
                "palmistry": (
                    "✋ ХИРОМАНТИЯ",
                    "Чтение судьбы по ладоням. Узнай свой врожденный потенциал и текущую реализацию через глубокий анализ линий твоей руки.\n\nСтоимость: 1200 ✨",
                    "biometrics",
                    1200,
                ),
                "sigil": (
                    "🎨 СИГИЛ-МАСТЕР: ГЕНЕРАТОР ЗНАКОВ УДАЧИ",
                    "Трансформируй свое желание в уникальный графический символ - сигил по древним правилам практической магии. Сохрани его на экран телефона для активации потоков удачи!\n\nСтоимость: 1000 ✨",
                    "artifacts",
                    1000,
                ),
                "alchemist": (
                    "🧪 ЦИФРОВОЙ АЛХИМИК",
                    "Расчет ведущего первоэлемента (Философская Ртуть, Сера или Соль) на основе даты твоего рождения и составление персональной формулы баланса.\n\nСтоимость: 700 ✨",
                    "artifacts",
                    700,
                ),
                "karma": (
                    "🧬 КАРМИЧЕСКИЙ НАВИГАТОР",
                    "Узнай тайну своего предыдущего воплощения, невыполненные кармические долги и получи ключ к решению текущих жизненных трудностей через интерактивный тест.\n\nСтоимость: 900 ✨",
                    "destiny",
                    900,
                ),
                "totem": (
                    "🐾 ТОТЕМНЫЙ ПРОВОДНИК",
                    "Определение твоего священного животного силы через шаманский интерактивный квиз, послание от тотема и советы по использованию его энергии.\n\nСтоимость: 900 ✨",
                    "destiny",
                    900,
                ),
                "astro_geo": (
                    "🗺 АСТРО-КАРТОГРАФИЯ",
                    "Анализ влияния планетарных линий на земную географию. Введи город или страну и узнай, принесет ли это место тебе богатство, любовь или испытания.\n\nСтоимость: 900 ✨",
                    "destiny",
                    900,
                ),
                "egyptian_oracle": (
                    "🏺 ДРЕВНИЙ ЕГИПЕТСКИЙ ОРАКУЛ",
                    "Свитки Судьбы из Книги Тота. Три священных пророчества древних богов Гелиополя, раскрывающие векторы твоего триумфа.\n\nСтоимость: 700 ✨",
                    "oracles",
                    700,
                ),
                "shadow_oracle": (
                    "👤 ТЕНЕВОЙ ОРАКУЛ ЛИЛИТ",
                    "Встреча со своими истинными, подавленными желаниями и теневой стороной личности по концепции Юнга. Трансформируй своих демонов в огромный ресурс силы.\n\nСтоимость: 700 ✨",
                    "oracles",
                    700,
                ),
                "chrono": (
                    "⏱ ХРОНО-ПРОГНОЗ",
                    "Расчет личных 'ведьминских часов' и пиков мистической активности на месяц вперед. Идеальный эзотерический тайм-менеджмент.\n\nСтоимость: 700 ✨",
                    "oracles",
                    700,
                ),
                "charoslov": (
                    "🌾 СЛАВЯНСКИЙ ЧАРОСЛОВ",
                    "Старообрядческие заговоры, обереги и индивидуальные ведовские практики защиты/привлечения блага на сегодняшний день.\n\nСтоимость: 600 ✨",
                    "oracles",
                    600,
                ),
                "synastry": (
                    "❤️ СОВМЕСТИМОСТЬ",
                    "Глубокий анализ ваших отношений по звездам. Узнайте, созданы ли вы друг для друга, какие кармические задачи стоят перед вами как парой, а также раскройте потенциал вашей сексуальной и эмоциональной связи.\n\nСтоимость: 1500 ✨",
                    "energy_sync",
                    1500,
                ),
                "destiny_card": (
                    "⭐ КАРТА СУДЬБЫ",
                    "Твой главный жизненный путь на всю жизнь. Самый важный разбор в твоем профиле, определяющий твой архетип, предназначение и кармические задачи.\n\nСтоимость: 1500 ✨",
                    "energy_sync",
                    1500,
                ),
                "dream": (
                    "🌙 СОННИК",
                    "Толкование твоих снов через призму архетипов и символов. Узнай, что хочет сказать твое подсознание, и расшифруй тайные знаки ночи.\n\nСтоимость: 1000 ✨",
                    "energy_sync",
                    1000,
                ),
                "sex": (
                    "🔥 СЕКСУАЛЬНОСТЬ",
                    "Погружение в мир твоих истинных чувств, желаний и сексуального потенциала через анализ твоих планет страсти.\n\nСтоимость: 1000 ✨",
                    "energy_sync",
                    1000,
                ),
                "money": (
                    "💰 ДЕНЕЖНЫЙ ПОТОК",
                    "Раскрой свой путь к финансовой свободе, сильные и слабые стороны в материальном мире и способы привлечения изобилия.\n\nСтоимость: 900 ✨",
                    "energy_sync",
                    900,
                ),
                "shadow": (
                    "👹 ВАШИ ДЕМОНЫ / ТЕНЬ",
                    "Встреча с тем, что скрыто в глубине тебя. Осознание и принятие своей теневой стороны для обретения целостности.\n\nСтоимость: 700 ✨",
                    "energy_sync",
                    700,
                ),
                "final": (
                    "🧭 ВАШ ПУТЬ В ЖИЗНИ",
                    "Главный вектор твоей жизни, долгосрочные цели и предназначение души в этом воплощении.\n\nСтоимость: 1200 ✨",
                    "energy_sync",
                    1200,
                ),
                "oracle": (
                    "🔮 СПРОСИ У ЗВЕЗД",
                    "Ответ на твой самый важный и волнующий вопрос от звездного Оракула.\n\nСтоимость: 500 ✨",
                    "energy_sync",
                    500,
                ),
                "antitaro": (
                    "🃏 АНТИ-ТАРО",
                    "Честный, трезвый взгляд на ситуацию без розовых очков и иллюзий.\n\nСтоимость: 500 ✨",
                    "energy_sync",
                    500,
                ),
                "micro_insight": (
                    "✨ МИКРО-ИНСАЙТ",
                    "Быстрый и точный совет от твоего персонального Проводника на ближайший час.\n\nСтоимость: 100 ✨",
                    "energy_sync",
                    100,
                ),
            }
            title, desc, cat, cost = item_data.get(
                key, ("Услуга", "Описание", "biometrics", 700)
            )
            ...
            if user and is_vip_unlimited(user):
                if key == "palmistry":
                    cost = 0
                    desc = desc.replace(
                        "Стоимость: 1200 ✨",
                        "Вам доступно БЕСПЛАТНО по VIP-тарификации!",
                    )
                elif key == "dream":
                    cost = 0
                    desc = desc.replace(
            ...
            kb.add(
                Callback(
                    "✅ АКТИВИРОВАТЬ РИТУАЛ",
                    payload={"cmd": "confirm_buy", "type": "service", "key": key},
                ),
                color=KeyboardButtonColor.POSITIVE,
            )
            kb.row()
            kb.add(
                Callback(
                    "⬅️ НАЗАД В КАБИНЕТ", payload={"cmd": "secret_arts_cat", "cat": cat}
                ),
                color=KeyboardButtonColor.SECONDARY,
            )

            att = await upload_local_photo(
                bot.api, "uslugi/services.jpeg", peer_id=vk_id
            )
            await ghost_edit(
                bot.api,
                peer_id,
            ...
            val = payload.get("val")
            await set_user_state(
                vk_id, json.dumps({"step": "waiting_totem_step2", "time_of_day": val})
            )
            ...
            time_of_day = (
                state.get("time_of_day", "☀️ Полдень") if state else "☀️ Полдень"
            )
            await set_user_state(vk_id, "")
            ...
            val = payload.get("val")
            await set_user_state(
                vk_id, json.dumps({"step": "waiting_karma_step2", "symbol": val})
            )
            ...
            kb_json = get_natal_chart_inline_keyboard(
                user.get("purchased_sections", {})
            )
            att = await upload_local_photo(
                bot.api, "uslugi/services.jpeg", peer_id=vk_id
            )
            await ghost_edit(
                bot.api,
                peer_id,
                "🔮 ТВОЯ ЗВЕЗДНАЯ КАРТА\n\nВыбери раздел для глубокого погружения. Каждый разбор можно получить один раз.",
                conversation_message_id=obj.get("conversation_message_id"),
                keyboard=kb_json,
                attachment=att,
            )
            ...
            await show_admin_console(
                peer_id, conversation_message_id=obj.get("conversation_message_id")
            )
        elif cmd == "guide_menu" or cmd == "guide":
            await show_guide_logic(
                vk_id,
                peer_id,
                skip_lock=True,
                conversation_message_id=conv_id
                if (conv_id := obj.get("conversation_message_id"))
                else None,
            )
        elif cmd == "guide_energy":
            await show_guide_energy_logic(
                vk_id,
                peer_id,
                conversation_message_id=obj.get("conversation_message_id"),
                skip_lock=True,
            )
        elif cmd == "guide_services":
            await show_guide_services_logic(
                vk_id,
                peer_id,
                conversation_message_id=obj.get("conversation_message_id"),
                skip_lock=True,
            )
        elif cmd == "guide_syndicate":
            await show_guide_syndicate_logic(
                vk_id,
                peer_id,
                conversation_message_id=obj.get("conversation_message_id"),
                skip_lock=True,
            )
        elif cmd == "guide_grimoire":
            await show_guide_grimoire_logic(
                vk_id,
                peer_id,
                conversation_message_id=obj.get("conversation_message_id"),
                skip_lock=True,
            ...
            await settings_choose_character(
                vk_id=vk_id,
                peer_id=peer_id,
                skip_lock=True,
                idx=payload.get("idx", 0),
            ...
            await process_skin_action_logic(
                vk_id=vk_id,
                peer_id=peer_id,
                skip_lock=True,
                payload=payload,
            ...
            await reg_mod.process_onboarding_skin_logic(
                vk_id,
                peer_id,
                payload.get("skin"),
                conversation_message_id=obj.get("conversation_message_id"),
            ...
            if not latest_data or "text" not in latest_data:
                await bot.api.messages.send(
                    peer_id=peer_id,
                    message="🔮 ДАННЫЕ СТЕРТЫ В ЦЕЛЯХ БЕЗОПАСНОСТИ\n\nТекст разбора был очищен. Сгенерируйте разбор заново в разделе Услуг.",
                    random_id=random.getrandbits(63),
                )
                return
            ...
            await bot.api.messages.send(
                peer_id=peer_id,
                message="Создаю PDF-файл, подожди секунду...",
                random_id=random.getrandbits(63),
            )
            ...
            u_name = user.get("first_name") or user.get("purchased_sections", {}).get(
                "first_name", "Адепт"
            )
            ...
            try:
                ...
                if success and os.path.exists(pdf_name):
                    ...
                    if not doc:
                        await bot.api.messages.send(
                            peer_id=peer_id,
                            message="Ошибка при загрузке PDF в систему ВК. Попробуйте позже.",
                            random_id=random.getrandbits(63),
                        )
                        return
                    ...
                    await bot.api.messages.send(
                        peer_id=peer_id,
                        message="Твой PDF-файл готов. Ты можешь сохранить его или поделиться с друзьями:",
                        attachment=doc,
                        random_id=random.getrandbits(63),
                        keyboard=kb,
                    )
                    ...
                    await bot.api.messages.send(
                        peer_id=peer_id,
                        message="Ошибка при создании PDF. Пожалуйста, попробуйте позже.",
                        random_id=random.getrandbits(63),
                    )
            finally:
                ...
                for tmp_f in temp_files_to_clean:
                    if os.path.exists(tmp_f):
                        try:
                            ...
                            logger.error(
                                f"Error removing recreated temp file {tmp_f}: {e}"
                            )
                if os.path.exists(pdf_name):
                    try:
                        ...
                        logger.error(
                            f"Error removing generated PDF file {pdf_name}: {e}"
                        )
            ...
            if action == "settings":
                ...
                await set_user_state(
                    vk_id,
                    json.dumps({"step": "waiting_birth_date", "conv_id": conv_id}),
                )
                kb = Keyboard(inline=True).add(
                    Callback(
                        "ОТМЕНА",
                        payload={"cmd": "profile_action", "action": "settings"},
                    ),
                    color=KeyboardButtonColor.NEGATIVE,
                )
                att = await upload_local_photo(
                    bot.api, "uslugi/settings.jpeg", peer_id=vk_id
                )
                await ghost_edit(
                    bot.api,
                    peer_id,
                    conversation_message_id=conv_id,
                    message="Для калибровки звездного пути напиши свою ДАТУ рождения (например, 15.04.1990). Данные будут храниться 24 часа для проведения ритуалов.",
                    keyboard=kb.get_json(),
                    attachment=att,
                )
            elif action == "change_skin":
                await settings_choose_character(
                    vk_id=vk_id, peer_id=peer_id, skip_lock=True, edit_msg_id=conv_id
                )
                ...
                await safe_edit(
                    peer_id=peer_id,
                    conversation_message_id=conv_id,
                    message="Шепот звезд успешно отключен. Ты больше не будешь получать ежедневные послания, но доступ к услугам подписки сохранен.",
                    keyboard=settings_menu_kb(vk_id, is_muted=True),
                )
                ...
                await safe_edit(
                    peer_id=peer_id,
                    conversation_message_id=conv_id,
                    message="Шепот звезд снова активен! Завтра ты получишь новое послание.",
                    keyboard=settings_menu_kb(vk_id, is_muted=False),
                )
            elif action == "reset_account":
                await set_user_state(
                    vk_id, json.dumps({"step": "waiting_reset_confirm"})
                )
                kb = (
                ...
                await ghost_edit(
                    ...
                    peer_id,
                    conversation_message_id=conv_id,
                    message="⚠️ ВНИМАНИЕ: Это действие безвозвратно удалит все ваши данные, покупки и прогресс в системе. Вы уверены?",
                    keyboard=kb.get_json(),
                    attachment=att,
                )
                ...
                await update_user(
                    vk_id, {"readings_history": [], "tags": [], "core_profile": ""}
                )
                ...
                await safe_edit(
                    peer_id=peer_id,
                    conversation_message_id=conv_id,
                    message="Твои личные данные и история полностью стерты. Твой путь чист, но сила звезд (баланс) осталась с тобой.",
                )
                ...
                await show_guide_logic(
                    vk_id, peer_id, skip_lock=True, conversation_message_id=conv_id
                )
            elif action == "admin_console":
                await show_admin_console(peer_id, conversation_message_id=conv_id)
            elif action == "syndicate":
                await syndicate_dashboard_logic(
                    vk_id=vk_id,
                    peer_id=peer_id,
                    skip_lock=True,
                    conversation_message_id=conv_id,
                ...
                kb = Keyboard(inline=True).add(
                    Callback(
                        "Отмена",
                        payload={"cmd": "profile_action", "action": "cancel_seal"},
                    ),
                    color=KeyboardButtonColor.NEGATIVE,
                )
                await safe_edit(
                    ...
                    try:
                        ...
                        if bdate and city:
                            ...
                            kb.row().add(
                                Callback(
                                    "🔄 ИЗМЕНИТЬ",
                                    payload={"cmd": "edit_onboarding_data"},
                                ),
                                color=KeyboardButtonColor.NEGATIVE,
                            )
            ...
            if buy_type in ["abandoned_10", "abandoned_15"]:
                amount_needed = int(
                    amount_needed * (0.90 if buy_type == "abandoned_10" else 0.85)
                )
                buy_type = (
                    "service"
                    if key
            ...
            if buy_type == "topup" or key.startswith("topup_"):
                ...
                p["last_cart_stage"] = 0
                p["last_cart_at"] = datetime.datetime.now(
                ...
                if payment_url:
                    kb = Keyboard(inline=True).add(
                        OpenLink(link=payment_url, label="💳 ОПЛАТИТЬ КАРТОЙ")
                    )
                    kb.row().add(
            ...
            if balance >= amount_needed:
                ...
                if key == "oracle_upsell":
                    key = "oracle"  # resolve the upsell back to its base service
                    ...
                    if event_id:
                        await bot.api.messages.send_message_event_answer(
                            ...
                            event_data=json.dumps(
                                {
                                    "type": "show_snackbar",
                                    "text": f"🎭 {char_name} теперь твой Проводник! ✨",
                                }
                            ),
                        )
                    return
                ...
                if buy_type == "service":
                    ...
                    asyncio.create_task(
                        add_event(vk_id, "paid_feature_used", {"service": key})
                    )
                    await process_payment_and_generate(
                        vk_id,
                        key,
                        peer_id=peer_id,
                        conversation_message_id=obj.get("conversation_message_id"),
                    ...
                    if event_id:
                        await bot.api.messages.send_message_event_answer(
                            ...
                            event_data=json.dumps(
                                {
                                    "type": "show_snackbar",
                                    "text": f"✨ Списано {amount_needed} энергии. Ритуал начат!",
                                }
                            ),
                        )
                    ...
                    if key == "tariff_vip":
                        logger.warning(
                            f"USER {vk_id} PURCHASED VIP_ADEPT TARIFF (5900 energy) - UNLIMITED ACTIVATED"
                        )
                        p = user.get("purchased_sections", {})
                        for s in [
                            "sex",
                            ...
                            "final",
                            "all",
                            "destiny_card_purchased",
                            "synastry",
                        ]:
                            p[s] = True
                        updates["purchased_sections"], updates["has_full_chart"] = (
                            p,
                            True,
                        )
                        # Безлимит на 30 дней
                        vip_expires = datetime.datetime.now(
                            datetime.timezone.utc
                    ...
                    if event_id:
                        await bot.api.messages.send_message_event_answer(
                            ...
                            event_data=json.dumps(
                                {
                                    "type": "show_snackbar",
                                    "text": f"✨ Транзит активирован! Баланс: {new_balance} ✨",
                                }
                            ),
                        )
                ...
                if event_id:
                    await bot.api.messages.send_message_event_answer(
                        ...
                        event_data=json.dumps(
                            {
                                "type": "show_snackbar",
                                "text": "🛑 Недостаточно энергии для активации!",
                            }
                        ),
                    )
                ...
                kb.add(
                    Callback(
                        "🎁 ПОЗВАТЬ ДРУГА (+500 ✨)", payload={"cmd": "get_referral"}
                    ),
                    color=KeyboardButtonColor.POSITIVE,
                ).row()
                kb.add(
                    OpenLink(
                        link="https://vk.com/@taroanti-oferta",
        ...
        elif cmd in ["grimoire_page", "grimoire_cards_list"]:
            await show_grimoire_page(
                vk_id,
                peer_id,
                payload.get("page", 0),
                skip_lock=True,
            ...
            if user:
                ...
                if purchased.get("dream"):
                    ...
                    await bot.state_dispenser.set(
                        ...
                        raw_json=json.dumps({"step": "waiting_dream_text"}),
                    )
                    msg = (
                        "Расскажи мне свой сон подробным текстом.\n\n"
                        "Можно добавить:\n"
            ...
            await show_tariffs(
                vk_id, peer_id, 0, edit_msg_id=obj.get("conversation_message_id")
            )
            ...
            await delete_bot_message(
                bot.api, peer_id, cmid=obj.get("conversation_message_id")
            )
        elif cmd == "set_rating":
            ...
            if state and state.get("step") == "waiting_feedback_comment":
                ...
                kb = Keyboard(inline=True)
                kb.add(
                    Callback("🔮 Новый расклад", payload={"cmd": "services_menu"}),
                    color=KeyboardButtonColor.POSITIVE,
                )
                kb.row()
                kb.add(
                    Callback("🏠 Главное меню", payload={"cmd": "main_menu"}),
                    color=KeyboardButtonColor.SECONDARY,
                )
            ...
            await safe_edit(
                peer_id=peer_id,
                conversation_message_id=obj.get("conversation_message_id"),
                message="🕯 Время скрыто, но это не помешает нам. И последний штрих — в каком ГОРОДЕ ты увидел свой первый звездный свет?",
            )
        elif cmd == "oracle_cut":
            state = await get_fsm_step(vk_id)
            ...
            await safe_edit(
                peer_id=peer_id,
                message="✨ ШАГ 3 ИЗ 3: ТВОЙ ВЫБОР ✨\nПрислушайся к интуиции и выбери 3 карты, которые откликаются тебе сейчас.",
                conversation_message_id=obj.get("conversation_message_id"),
                keyboard=kb.get_json(),
            )
        elif cmd == "global_cut":
            ...
            await safe_edit(
                peer_id=peer_id,
                conversation_message_id=conv_id,
                message="✦ КАРТА ВЫБРАНА. ИНИЦИАЦИЯ...",
                keyboard=Keyboard(inline=True).get_json(),
            )
            ...
            if user:
                ...
                if not isinstance(unlocked, dict):
                    unlocked = (
                        {str(c): "Первое касание" for c in unlocked}
                        if isinstance(unlocked, list)
                        else {}
                    )
                if card_id not in unlocked:
                    unlocked[card_id] = (
                        f"{card_data.get('name', 'Карта')} - {card_data.get('subtitle', 'Новое знание')}"
                    )
                await update_user(
                    vk_id,
                    {
            ...
            skin_att = await upload_local_photo(
                bot.api, SKIN_ASSETS.get(active_skin, "o.png"), peer_id=vk_id
            )
            card_att = await upload_local_photo(
                bot.api, f"{card_id}.jpeg", peer_id=vk_id
            )

            atts = []
            if skin_att:
                atts.append(skin_att)
            if card_att:
                atts.append(card_att)
