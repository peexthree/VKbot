...
async def process_city(message: Message):
    ...
    try:
        ...
        try:
            await message.answer(
                "Я закончила изучение твоей точки входа в этот мир. Теперь система знает о тебе больше, чем ты сам. Я подготовила для тебя 70 приветственных бонусов и твой первый анализ. Нажми кнопку ниже, чтобы забрать подарок и начать путь.",
                keyboard=json.dumps(kb, ensure_ascii=False)
            )
        except Exception as e:
            print(f"Error sending sync completion message: {e}")
            await message.answer("Я закончила изучение твоей точки входа в этот мир. Теперь система знает о тебе больше, чем ты сам. Я подготовила для тебя 70 приветственных бонусов и твой первый анализ. Нажми кнопку ниже, чтобы забрать подарок и начать путь.")
