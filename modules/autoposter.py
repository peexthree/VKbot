...
async def generate_post():
    ...
    prompt = (
        ...
        f"2. Живой хук: Начни текст с того что ты представишься пользователям,далее с  упоминания текущего дня недели ({current_day}) и свяжи его с типичным разрушительным поведением людей сегодня.\n"
        f"3. Вскрытие боли: Резко перейди к сути темы «{topic}». Покажи, как люди ломают свою судьбу из-за страхов и иллюзий.\n"
        f"4. Запретное знание: Выдай одно жесткое эзотерическое правило по этой теме как непреложный закон, расскажи позитивные моменты.\n"
        f"5. Вовлечение: Закончи пост неудобным вопросом, который заденет гордость читателя и заставит спорить в комментариях.\n\n"
        f"Технические требования:\n"
        f"- Текст должен быть коротким,инфогрмативным и бить точно в цель.\n"
