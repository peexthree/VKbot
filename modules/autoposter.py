...
async def generate_post(is_morning: bool = True, forced_rubric: str = None):
    ...
    rubric_instructions = {
        ...
        "PRACTICUM": (
            f"Жесткий практикум по вскрытию болей. Объем: {jitter_length}. Структура: {jitter_style}. "
            "Вместо сухих инструкций - дерзкий пошаговый разбор. Дай 3-5 конкретных шагов. "
            "Пиши максимально нагло. Используй списки с короткими дефисами (-) или магическими символами."
        ),
        "SUPPORT": (
            f"Сакральная поддержка. Единственная рубрика без жесткого цинизма. Объем: {jitter_length}. "
            "Прояви глубокое психологическое сочувствие боли читателя. Персонаж остается собой (со своими маркерами), "
            "но направляет силу на мягкую поддержку. Дай совет, как трансформировать внутренний хаос в ресурс."
        ),
        "FACT": (
        ...
        "POLL": (
            ...
            "Текст-интрига по теме, который обрывается на самом интересном месте и заставляет нажать на вариант в опросе ниже."
        ),
        "NEWS_BREAKDOWN": (
