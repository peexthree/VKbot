...
SKIN_MAP = {
    ...
    "Магистр": "Ты - Великий Магистр, владыка всех 22 Старших Арканов. Твой стиль - абсолютное знание, власть над реальностью, строгая и непоколебимая мудрость, превосходство и мистическая сила."
}
...
async def generate_text(prompt: str, json_mode: bool = False, skin: str = "olesya") -> str | None:
    # 18+ Пре-фильтрация
    stop_words = ["порно", "инцест", "педофилия", "зоофилия", "расчлененка", "насилие", "убийство", "суицид", "изнасилование"]
    prompt_lower = prompt.lower()
    for word in stop_words:
        if word in prompt_lower:
            return "Твои мысли окутаны тьмой, к которой я не хочу прикасаться. Сформулируй свой запрос иначе, если действительно ищешь света."
...
async def extract_tags(text: str) -> list[str]:
    prompt = (
        ...
        f"Выбери 1-3 тега, которые максимально точно описывают текущую ситуацию пользователя, СТРОГО из следующего списка:\n"
        f"['кризис_финансов', 'измена', 'поиск_предназначения', 'выгорание', 'одиночество', 'карьерный_тупик', "
        f"'токсичные_отношения', 'страх_будущего', 'неуверенность', 'потеря_смысла', 'конфликт_с_родными', "
        f"'апатия', 'долги', 'проблемы_со_здоровьем_из_за_стресса', 'развод', 'зависимость_от_партнера', "
        f"'творческий_кризис', 'страх_осуждения', 'прокрастинация', 'обида_на_прошлое'].\n"
        f"Не придумывай свои теги. Если ничего не подходит идеально, выбери наиболее близкие. "
        f"Верни строго JSON-список строк, например: [\"кризис_финансов\", \"поиск_предназначения\"]."
    )
